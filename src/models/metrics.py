...
def calculate_silhouette_score(data,labels,metric='euclidean'):
    """
   The calculate_silhouette_score function computes the Silhouette Score for a given set of data points and
   their corresponding cluster labels. The Silhouette Score is a metric used to assess the quality of clusters
    by measuring the separation between clusters and the similarity within clusters.

    Parameters:

    data: A data matrix or DataFrame containing the data points to be clustered.

    labels: A list or array of cluster labels corresponding to each data point.

    metric: A string specifying the distance metric to be used when calculating the Silhouette Score.
    Defaults to 'euclidean'. This parameter determines how distances between data points are computed.

    Returns:

    A float representing the computed Silhouette Score for the given data and cluster labels.
    The score typically ranges from -1 to 1, where higher values indicate better-defined clusters.
    """
    score = silhouette_score(data, labels, metric=metric)
    return score


# Source : https://github.com/christopherjenness/DBCV
